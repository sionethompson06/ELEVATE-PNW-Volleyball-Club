...
export async function PATCH(request: Request, context: RouteContext) {
  ...
  if (Object.keys(updatePayload).length === 0) {
    return NextResponse.json(
      { error: "No valid player fields provided for bulk update." },
      { status: 400 }
    );
  }
  ...
  if (error) {
    return NextResponse.json(
      { error: `Failed to bulk update family players: ${error.message}` },
      { status: 500 }
    );
  }
  ...
  return NextResponse.json({
    success: true,
    message: "Family players updated successfully.",
  });
}
