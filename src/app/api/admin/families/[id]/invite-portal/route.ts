...
export async function POST(
  ...
  const [{ data: family, error: familyError }, { data: players, error: playersError }] =
    await Promise.all([
      supabase
        .from("families")
        .select("id, family_name, primary_parent_email, account_status")
        .eq("id", id)
        .single(),
      supabase
        .from("players")
  ...
  const { error: updateError } = await supabase
    .from("families")
    .update({ account_status: "portal_invited" })
    .eq("id", id);

  if (updateError) {
    return NextResponse.json(
      { error: "Unable to record portal invitation." },
      { status: 500 }
    );
  }
